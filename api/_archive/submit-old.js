...
// If on Vercel, also allow current deployment
...
function isOriginAllowed(origin) {
  if (!origin) {
    console.warn('No origin provided in request');
    return false;
  }
  
  // Extract domain from origin (handle URLs)
  let originDomain = origin;
  try {
    const url = new URL(origin);
    originDomain = url.origin;
  } catch (e) {
    // If it's not a valid URL, try to add https:// and parse
    try {
  ...
  console.log('Checking origin:', originDomain, 'Against allowed:', ALLOWED_ORIGINS.join(', '));
  ...
  const allowed = ALLOWED_ORIGINS.some(allowedOrigin => {
    ...
    return originDomain === normalizedAllowed || 
           originDomain.endsWith('.' + normalizedAllowed.replace(/^https?:\/\//, '')) ||
           originDomain.endsWith(normalizedAllowed.replace(/^https?:\/\//, ''));
  });
  
  if (!allowed) {
    console.warn('Origin rejected:', originDomain);
  }

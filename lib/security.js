...
function isOriginAllowed(origin) {
  // In development, allow requests with no Origin header (curl, Postman, PowerShell, etc.)
  if (!origin) {
  ...
  if (config.api.allowedOrigins.length === 0) {
    console.warn('⚠️  ALLOWED_ORIGINS not configured!');
    if (config.isDevelopment()) {
      return normalizedOrigin.includes('localhost') || 
             normalizedOrigin.includes('127.0.0.1');
    }
    // In production, if VERCEL_URL is set, allow that
    if (config.vercelUrl) {
      const vercelOrigin = `https://${config.vercelUrl}`.toLowerCase();
      console.log(`📝 Fallback: Allowing Vercel URL: ${vercelOrigin}`);
      return normalizedOrigin === vercelOrigin || 
             normalizedOrigin.startsWith(vercelOrigin);
    }
    return false;
  }

  // Always allow any localhost / 127.0.0.1 origin regardless of port.
  // These origins can never appear in production (no real user browses from localhost),
  // so this is safe and also covers VS Code Live Server (5500), BrowserSync, etc.
  if (normalizedOrigin.startsWith('http://localhost:') ||
      normalizedOrigin.startsWith('http://127.0.0.1:')) {
    return true;
  }

  const isAllowed = config.api.allowedOrigins.some(allowed => {
    const normalizedAllowed = allowed.toLowerCase().replace(/\/$/, '');
    return normalizedOrigin === normalizedAllowed || 
           normalizedOrigin.startsWith(normalizedAllowed);
  });
  
  console.log(`🔐 Origin check: ${origin} -> ${isAllowed ? 'ALLOWED' : 'DENIED'}`);
...
/**
 * Lenient rate limiter for read-only endpoints (GET /api/get-slots).
 * Users reload the booking page freely — use a much higher ceiling.
 * Default: 120 reads per hour per IP.
 */
const readRateLimiter = new RateLimiter(
  parseInt(process.env.API_READ_RATE_LIMIT_MAX || '120', 10),
  config.api.rateLimitWindowMs
);

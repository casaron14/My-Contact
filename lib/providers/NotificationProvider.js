...
class TelegramProvider extends NotificationProvider {
  ...
  buildBookingMessage(bookingData) {
    const slotDate = new Date(bookingData.slotDateTime).toLocaleString('en-GB', { // en-GB locale uses 24-hour format
      weekday: 'short',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
  ...
  /**
   * Send alert for a new seminar sign-up
   */
  async sendSeminarAlert(leadData) {
    if (!config.telegram.enabled) {
      console.warn('[DEV MODE] Telegram seminar alert skipped');
      return { success: true, isDev: true };
    }

    try {
      const waLink  = `https://wa.me/${leadData.phone.replace(/\D/g, '')}`;
      const message =
        `📋 *NEW SEMINAR SIGN\-UP*\n\n` +
        `*Name:* ${this.escape(leadData.name || '—')}\n` +
        `*Email:* ${leadData.email}\n` +
        `*Phone:* ${leadData.phone}\n` +
        `*Location:* ${this.escape(leadData.location)}\n\n` +
        `[📱 Message on WhatsApp](${waLink})`;

      return await this.sendMessage(message);
    } catch (error) {
      console.error('✗ Telegram seminar alert failed:', error.message);
      return { success: false, error: error.message };
    }
  }
...
class ConsoleProvider extends NotificationProvider {
  ...
  async sendSeminarAlert(leadData) {
    console.log('📋 [NOTIFICATION] Seminar Sign-Up:');

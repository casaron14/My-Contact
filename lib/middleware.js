...
const { isOriginAllowed, getClientIp, rateLimiter, ApiError, formatErrorResponse } = require('./security');
...
function createSecurityMiddleware() {
  return async (req, res) => {
    ...
    if (!isOriginAllowed(origin) && req.method !== 'OPTIONS') {
      ...
      console.warn(`   Allowed origins: ${config.api.allowedOrigins.join(', ')}`);
      console.warn(`   Raw headers - Origin: ${req.headers.origin}, Referer: ${req.headers.referer}`);
    ...
    res.setHeader('X-RateLimit-Limit', config.api.rateLimitMax);
